...
public class TpccStatements {
    ...
    public TpccStatements(Connection conn, int fetchSize) throws Exception {
        ...
        pStmts[0]  = prepareStatement("SELECT c.c_discount, c.c_last, c.c_credit, w.w_tax FROM default AS c JOIN default AS w ON KEYS c.c_w_id_ref WHERE c.type = 'customer' AND w.type = 'warehouse' AND META(w).id = 'warehouse_' || ? AND c.c_w_id = ? AND c.c_d_id = ? AND c.c_id = ?"); // c.c_w_id in there multiple times but matches w_id?
        //pStmts[1]  = prepareStatement("SELECT d_next_o_id, d_tax FROM district WHERE d_id = ? AND d_w_id = ? FOR UPDATE");
        pStmts[1]  = prepareStatement("SELECT d_next_o_id, d_tax FROM default AS district WHERE type = 'district' AND d_id = ? AND d_w_id = ?");
        //pStmts[2]  = prepareStatement("UPDATE district SET d_next_o_id = ? + 1 WHERE d_id = ? AND d_w_id = ?");
        pStmts[2]  = prepareStatement("UPDATE default SET d_next_o_id = ? + 1 WHERE type = 'district' AND d_id = ? AND d_w_id = ?");
        //pStmts[3]  = prepareStatement("INSERT INTO orders (o_id, o_d_id, o_w_id, o_c_id, o_entry_d, o_ol_cnt, o_all_local) VALUES(?, ?, ?, ?, ?, ?, ?)");
        pStmts[3]  = prepareStatement("INSERT INTO default VALUES (?, {\"type\": \"orders\", \"o_id\": ?, \"o_d_id\": ?, \"o_w_id\": ?, \"o_c_id\": ?, \"o_entry_d\": ?, \"o_ol_cnt\": ?, \"o_all_local\": ?})");
        //pStmts[4]  = prepareStatement("INSERT INTO new_orders (no_o_id, no_d_id, no_w_id) VALUES (?,?,?)");
        pStmts[4]  = prepareStatement("INSERT INTO default VALUES (?, {\"type\": \"new_orders\", \"no_o_id\": ?, \"no_d_id\": ?, \"no_w_id\": ?})");
        //pStmts[5]  = prepareStatement("SELECT i_price, i_name, i_data FROM item WHERE i_id = ?");
        pStmts[5]  = prepareStatement("SELECT i_price, i_name, i_data FROM default WHERE type = 'item' AND i_id = ?");
        ...
        pStmts[6]  = prepareStatement("SELECT s_quantity, s_data, s_dist_01, s_dist_02, s_dist_03, s_dist_04, s_dist_05, s_dist_06, s_dist_07, s_dist_08, s_dist_09, s_dist_10 FROM default WHERE type = 'stock' AND s_i_id = ? AND s_w_id = ?");
        //pStmts[7]  = prepareStatement("UPDATE stock SET s_quantity = ? WHERE s_i_id = ? AND s_w_id = ?");
        pStmts[7]  = prepareStatement("UPDATE default SET s_quantity = ? WHERE type = 'stock' AND s_i_id = ? AND s_w_id = ?");
        ...
        pStmts[8]  = prepareStatement("INSERT INTO default VALUES (?, {\"type\": \"order_line\", \"ol_o_id\": ?, \"ol_d_id\": ?, \"ol_w_id\": ?, \"ol_number\": ?, \"ol_i_id\": ?, \"ol_supply_w_id\": ?, \"ol_quantity\": ?, \"ol_amount\": ?, \"ol_dist_info\": ?})");

        // Payment statements.
        //pStmts[9]  = prepareStatement("UPDATE warehouse SET w_ytd = w_ytd + ? WHERE w_id = ?");
        pStmts[9]  = prepareStatement("UPDATE default SET w_ytd = w_ytd + ? WHERE type = 'warehouse' AND w_id = ?");
        //pStmts[10] = prepareStatement("SELECT w_street_1, w_street_2, w_city, w_state, w_zip, w_name FROM warehouse WHERE w_id = ?");
        pStmts[10] = prepareStatement("SELECT w_street_1, w_street_2, w_city, w_state, w_zip, w_name FROM default WHERE type = 'warehouse' AND w_id = ?");
        //pStmts[11] = prepareStatement("UPDATE district SET d_ytd = d_ytd + ? WHERE d_w_id = ? AND d_id = ?");
        pStmts[11] = prepareStatement("UPDATE default SET d_ytd = d_ytd + ? WHERE type = 'district' AND d_w_id = ? AND d_id = ?");
        //pStmts[12] = prepareStatement("SELECT d_street_1, d_street_2, d_city, d_state, d_zip, d_name FROM district WHERE d_w_id = ? AND d_id = ?");
        pStmts[12] = prepareStatement("SELECT d_street_1, d_street_2, d_city, d_state, d_zip, d_name FROM default WHERE type = 'district' AND d_w_id = ? AND d_id = ?");
        //pStmts[13] = prepareStatement("SELECT count(c_id) FROM customer WHERE c_w_id = ? AND c_d_id = ? AND c_last = ?");
        pStmts[13] = prepareStatement("SELECT count(c_id) FROM default where type = 'customer' AND c_w_id = ? AND c_d_id = ? AND c_last = ?");
        //pStmts[14] = prepareStatement("SELECT c_id FROM customer WHERE c_w_id = ? AND c_d_id = ? AND c_last = ? ORDER BY c_first");
        pStmts[14] = prepareStatement("SELECT c_id FROM default WHERE type = 'customer' AND c_w_id = ? AND c_d_id = ? AND c_last = ? ORDER BY c_first");
        ...
        pStmts[15] = prepareStatement("SELECT c_first, c_middle, c_last, c_street_1, c_street_2, c_city, c_state, c_zip, c_phone, c_credit, c_credit_lim, c_discount, c_balance, c_since FROM default WHERE type = 'customer' AND c_w_id = ? AND c_d_id = ? AND c_id = ?");
        //pStmts[16] = prepareStatement("SELECT c_data FROM customer WHERE c_w_id = ? AND c_d_id = ? AND c_id = ?");
        pStmts[16] = prepareStatement("SELECT c_data FROM default WHERE type = 'customer' AND c_w_id = ? AND c_d_id = ? AND c_id = ?");
        ...
        pStmts[20] = prepareStatement("SELECT count(c_id) FROM default WHERE type = 'customer' AND c_w_id = ? AND c_d_id = ? AND c_last = ?");
        ...
        pStmts[27] = prepareStatement("SELECT o_c_id FROM default WHERE type = 'orders' AND o_id = ? AND o_d_id = ? AND o_w_id = ?");
        //pStmts[28] = prepareStatement("UPDATE orders SET o_carrier_id = ? WHERE o_id = ? AND o_d_id = ? AND o_w_id = ?");
        pStmts[28] = prepareStatement("UPDATE default SET o_carrier_id = ? WHERE type = 'orders' AND o_id = ? AND o_d_id = ? AND o_w_id = ?");
        //pStmts[29] = prepareStatement("UPDATE order_line SET ol_delivery_d = ? WHERE ol_o_id = ? AND ol_d_id = ? AND ol_w_id = ?");
        pStmts[29] = prepareStatement("UPDATE default SET ol_delivery_d = ? WHERE type = 'order_line' AND ol_o_id = ? AND ol_d_id = ? AND ol_w_id = ?");
        //pStmts[30] = prepareStatement("SELECT SUM(ol_amount) FROM order_line WHERE ol_o_id = ? AND ol_d_id = ? AND ol_w_id = ?");
        pStmts[30] = prepareStatement("SELECT SUM(ol_amount) FROM default WHERE type = 'order_line' AND ol_o_id = ? AND ol_d_id = ? AND ol_w_id = ?");
        ...
        pStmts[31] = prepareStatement("UPDATE default SET c_balance = c_balance + ? , c_delivery_cnt = c_delivery_cnt + 1 WHERE type = 'customer' AND c_id = ? AND c_d_id = ? AND c_w_id = ?");
        ...
        pStmts[34] = prepareStatement("SELECT count(*) FROM default WHERE type = 'stock' AND s_w_id = ? AND s_i_id = ? AND s_quantity < ?");
