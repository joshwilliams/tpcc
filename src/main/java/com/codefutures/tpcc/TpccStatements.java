...
public class TpccStatements {
    ...
    public TpccStatements(Connection conn, int fetchSize) throws Exception {
        ...
        pStmts[3]  = prepareStatement("INSERT INTO orders (o_id, o_d_id, o_w_id, o_c_id, o_entry_d, o_ol_cnt, o_all_local) VALUES(?, ?, ?, ?, TO_TIMESTAMP(?, 'YYYY-MM-DD HH24:MI:SS.FF'), ?, ?)");
        ...
        pStmts[19] = prepareStatement("INSERT INTO history(h_c_d_id, h_c_w_id, h_c_id, h_d_id, h_w_id, h_date, h_amount, h_data) VALUES(?, ?, ?, ?, ?, TO_TIMESTAMP(?, 'YYYY-MM-DD HH24:MI:SS.FF'), ?, ?)");
        ...
        pStmts[29] = prepareStatement("UPDATE order_line SET ol_delivery_d = TO_TIMESTAMP(?, 'YYYY-MM-DD HH24:MI:SS.FF') WHERE ol_o_id = ? AND ol_d_id = ? AND ol_w_id = ?");
